...
namespace Ashsvp
{
    public class SimcadeVehicleController : MonoBehaviour
    {
        ...
        // Switch between old and new Input system.
        [Header("Input Settings")]
        [SerializeField] private bool _useInputSystem;
        ...
        private void Update()
        {
            if (CanDrive && CanAccelerate)
            {
                if (_useInputSystem)
                {
                    accelerationInput = (_carInputHandler.AccelerateTriggered ? 1f : 0f) +
                        (_carInputHandler.DecelerateTriggered ? -1f : 0f);

                    brakeInput = _carInputHandler.BrakeTriggered ? 1f : 0f;
                    steerInput = (_carInputHandler.TurnRightTriggered ? 1f : 0f) +
                                 (_carInputHandler.TurnLeftTriggered ? -1f : 0f);
                }
                else
                {
        ...
        // Disables/Enables car's driving and acceleration
        public void EnableCarMovement(bool value)
        {
            CanDrive = value;
            CanAccelerate = value;
        }
